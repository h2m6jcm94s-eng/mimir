...
export async function discordWebhookRoutes(app: FastifyInstance) {
  ...
  app.post('/discord/:tenantId', async (request: FastifyRequest, reply) => {
    ...
    const body = discordRequest.rawBody ?? JSON.stringify(discordRequest.body);
    ...
    const { session, messages, job, tier } = await withTenantTransaction(tenantId, async (ctx) => {
      ...
      const idempotencyKey = `discord:${tenantId}:${message.userId}:${Date.now()}`;
    ...
    await startReplyWorkflow({
      ...
      userId: actor,
      jobId: job.id,
      idempotencyKey: `discord:${tenantId}:${message.userId}:${Date.now()}`,
      type: 'discord.chat',
      tier,
