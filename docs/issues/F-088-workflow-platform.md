...
**Tier:** Pro · **Priority:** P1 · **Status:** In progress (backend foundation done)
...
4. **AI optimizer** — `POST /v1/workflows/:id/optimize` suggests the right tier, agent role, and model for each action node and removes unreachable nodes.
...
- [x] Schema: `routine` workflow columns + `workflow_node`/`workflow_edge` tables.
- [x] Import a sample n8n workflow and persist it as a Mimir routine.
- [x] Generate a draft workflow from a description.
- [x] Optimize an existing workflow and persist the suggestion log.
- [ ] Build and save a workflow in the visual editor.
- [x] Execute a workflow and record per-node status.
